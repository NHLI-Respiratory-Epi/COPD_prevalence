...
/* Create log file */
capture log close
log using "`analysis_dir'/build_logs/CompactObservation", text replace
...
forvalues cohort = 1/`cohorts' {
	...
	forvalues part = 1/`split' {
		...
		use stata_data/Observation`cohort'_`part', clear
		
		count

		//remove any unneeded variables
		drop consid pracid obsid enterdate staffid parentobsid ///
		obstypeid numrangelow numrangehigh probobsid
		
		//remove implausible dates to free up space
		drop if obsdate < date("01/01/1920", "DMY")
	...
	save stata_data/Observation`cohort'_compact, replace
}
